...
interface ValidateCitationInput {
  citation: string;
}

export function validateCitation(db: Database.Database, input: ValidateCitationInput) {
  ...
  if (caseMatch) {
    ...
    const caseRow = db.prepare(
      'SELECT case_number, ecli, case_name, court, judgment_date FROM case_law WHERE UPPER(case_number) = ?'
