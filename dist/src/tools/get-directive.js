export function getDirective(db, input) {
    ...
    LIMIT 1
  `).get(input.identifier, `%${input.identifier}%`, input.identifier);
    if (!act) {
        const candidates = db.prepare(`
      SELECT short_title, celex_number FROM eu_acts WHERE act_type = 'directive' ORDER BY short_title
    `).all();
        throw new Error(`Directive not found: "${input.identifier}". Available directives: ${candidates.map((c) => c.short_title || c.celex_number).join(', ')}`);
    }
    const articleCount = db.prepare('SELECT COUNT(*) as c FROM articles WHERE act_id = ?').get(act.id).c;
  ...
  `).all(act.id);
    const transpositionSummary = {
        transposed: transposition.filter((t) => t.status === 'transposed').length,
        partial: transposition.filter((t) => t.status === 'partial').length,
        not_transposed: transposition.filter((t) => t.status === 'not_transposed').length,
        infringement: transposition.filter((t) => t.status === 'infringement').length,
