export function getArticle(db, input) {
    ...
    LIMIT 1
  `).get(input.act, input.act, input.act);
    if (!act) {
  ...
  `).get(act.id, input.article, `Article ${input.article}`);
    if (!article) {
        // List available articles for guidance
        const available = db.prepare(`
      SELECT article_number FROM articles WHERE act_id = ? ORDER BY id LIMIT 20
    `).all(act.id);
        throw new Error(`Article ${input.article} not found in ${act.short_title || act.title}. ` +
            `Available articles include: ${available.map((a) => a.article_number).join(', ')}...`);
    }
    return {
        act: {
            id: act.id,
