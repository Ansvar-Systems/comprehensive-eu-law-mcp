export function getRegulation(db, input) {
  ...
  `).get(input.identifier, `%${input.identifier}%`, input.identifier);
    ...
    `).all();
        throw new Error(`Regulation not found: "${input.identifier}". Available regulations: ${candidates.map((c) => c.short_title || c.celex_number).join(', ')}`);
    }
