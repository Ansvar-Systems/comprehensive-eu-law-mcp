// CJEU Landmark Case Law
import type Database from 'better-sqlite3';

export function seedCJEUCases(db: Database.Database): void {
  console.log('  Seeding CJEU case law...');

  const insertCase = db.prepare(
    `INSERT INTO case_law (case_number, ecli, case_name, court, judgment_date, subject_matter, keywords, headnotes, ruling_summary, cited_acts, importance_score)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );

  const cases: {n: string, ecli: string, name: string, court: string, date: string, subject: string, kw: string, head: string, ruling: string, cited: string, score: number}[] = [
    {
